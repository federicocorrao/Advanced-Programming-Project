...
namespace STF
{
    class Parser
    {
        // funzione ausiliaria: stampa l'albero sintattico 
    ...
    /* Table generation */
        ...
        private ActionTable GenerateActionTable()
        {
            string fixtureName = tempTable[0][0].Attribute;
            tempTable.RemoveAt(0);
            ActionTable resultTable = new ActionTable(fixtureName);

            foreach (List<Token> l in tempTable)
            {
                ActionRow r = new ActionRow(l[0].Attribute);
        ...
        private ExampleTable GenerateExampleTable()
        {
            string fixtureName = tempTable[0][0].Attribute;
            ...
            ExampleTable resultTable = new ExampleTable(fixtureName, argNames, argTypes);
    ...
    /* Non-Terminals Procedures */
        ...
        private NodeField ParseField()
        {
            ...
            if (Lookahead(TokenType.FieldEnd))
                f = new NodeField { Text = ParseEmpty() };
            else
                f = new NodeField { Text = ParseText() };
        ...
        // Un NodeIdentifier con attributo string.Empty rappresenta un <td> vuoto
        private NodeText ParseEmpty()
        {
            tempRow.Add(new Token(TokenType.Identifier, string.Empty));
            return new NodeIdentifier { Attribute = string.Empty };
        }
